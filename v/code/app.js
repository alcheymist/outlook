...
export class app extends outlook.page {
    ...
    async get_current_business() {
        ...
        if (businesses.length > 1) {
            ...
            const key_values = businesses.map(business => ({ name: business.id, value: business.name }));
            // 
            ...
            const Choice = new outlook.choices(this.config.general, key_values, "multiple", "business_id", "content", undefined);
            //
            //3. Open the POPUP to select a business.
            const business_ids = await Choice.administer();
            //
            ...
            const name = business_ids[0];
            //
            //Get the business id for the selected business.
            const get_business_id = (kvs, name) => {
                //
    ...
    async edit_table() {
        ...
        const subject = { dbname: this.config.app_subject.dbname, ename: this.config.app_subject.ename };
        //
