...
export abstract class app extends outlook.page{
   ...
   public dbname: string;
    //
    //This is the user database that's needed to support the sql work.
    public user_dbase?: schema.database;
    //
    ...
    constructor(
        ...
        //
        //Bring the subject from the config file to this app class.
        this.subject = config.app_subject;
        //
        //Bring the database name to this application for easy access.
        this.dbname = this.subject.dbname;
        //
    ...
    //
    //This are the two databases we will have support in the construction of the 
    //sql statements.
    get databases():{[index:string]:schema.database}{
        //
        const result: {[index:string]:schema.database}= {};
        //
        const dbname1 = this.dbase!.name;
        //
        result[dbname1] = this.dbase!;
        //
        const dbname2 = this.user_dbase!.name;
        //
        result[dbname2] = this.user_dbase!;
        //
        return result;
    }
    //
    ...
    async initialize(): Promise<void>{
        ...
        this.dbase = await this.get_dbase(this.dbname);
        //
        //Set the user database.
        this.user_dbase = await this.get_dbase(this.config.app_subject.dbname);
        // 
    ...
    //
    //Use the server to get the named database from php and convert it into a 
    //javascript version.
    public async get_dbase(dbname: string): Promise<schema.database> {
        //
        ...
        return new schema.database(idbase);
    }
    //
    ...
    async get_current_business():Promise<outlook.business> {
        ...
        if(businesses.length > 1) {
            ...
            const key_values:Array<{ name: string;value: string;}> = businesses.map(business =>({name: business.id, value: business.name}));
            // 
            //2. Use the pairs to create a new choices POPUP that returns a selected
            //table
            // syntax outlook.choices<i, a extends "single" | "multiple", r extends a extends "single" ? i : a extends "multiple" ? i[] : never>
            ...
            const business_ids: Array<string> | undefined = await Choice.administer();
            //
            //Get the value of the selected business.
            if(business_ids === undefined) throw new schema.mutall_error(`No business was selected for this ${this.user!.name}.`);
            //
            //For a single case we expect to get only one element in the array.
            // This is the business name.
            const name:string = business_ids[0];
            //
            //Get the business id for the selected business.
            const get_business_id = (kvs:Array<outlook.option<string>>, name:string):string=> {
                //
                //Map the key values to keys.
                const keys = kvs.map(key => (key.value));
                //
                //Get the index of the key that matches the given id.
                const index = keys.indexOf(name);
                //
                //Use the index to retrieve the key value
                const id = key_values[index];
                //
                //Extract and return the name. );
                return id.name;
            }
            //
            return {id:get_business_id(key_values,name), name};
        }
        //
    ...
    private async welcome_user(): Promise<void> {
        ...
        this.get_element("business_name").textContent = this.user!.business!.name;
        //
    ...
    async register(): Promise<Array<string> | undefined> {
        ...
        const Role = new outlook.choices(this.config.general, inputs,'multiple', "role_id");
        //
        //Get the user roles
    ...
    private get_subscription_data(): Array<quest.label> {
        ...
        if (this.user!.name === (undefined || null)) {
            throw new schema.mutall_error("You cannot login without an name");
        }
    ...
    async edit_table() {
        ...
        //2. Use the pairs to create a new choices POPUP that returns a selected
        //entity name
        const Choice = new outlook.choices(this.config.general, pairs,'single', "table",undefined, "#content");
        //
        ...
        const subject: schema.subject = {dbname: this.config.app_subject.dbname, ename: this.config.app_subject.ename};
        //
...
export interface Iconfig {
    ...
    //This is the entity(and its database) name that is initialy displayed in 
    //the subject panel of the application page.
    ...
    //explore for on the server (for security reasons)
    images:string ;
    ...
    limit: number;
    //
    //level one registration template.
    lv1: string;
    //
    //svg template. 
    svg: string;  
    //
    //This is the entity and database name that define the global list od
    //users.
